...
from Nethya.Handlers.validation import is_user_admin
...
from Nethya.Plugins import ALL_MODULES
from Nethya.__help__ import (
get_help, 
help_button, 
get_settings, 
settings_button, 
...
def start(update: Update, context: CallbackContext):
    ...
    [
        InlineKeyboardButton(
            text="➕️ Add me to your chat❤️️ ➕️", url=f"https://t.me/{context.bot.username}?startgroup=true"),
    ],
    [
        InlineKeyboardButton(text="⭐️Admin⭐️", callback_data="admin_back"),
        InlineKeyboardButton(
            text="⭐️Users⭐️", callback_data="user_back"
        ),
    ],
    [
    ...
    [
        InlineKeyboardButton(text="〥Helps & Commands❔〥", callback_data="help_back"),
    ],
]),
...
def Nethya_about_callback(update: Update, context: CallbackContext):
    first_name = update.effective_user.first_name
    query = update.callback_query
    if query.data == "Nethya_":
...
def main():
    ...
    about_callback_handler = CallbackQueryHandler(Nethya_about_callback, pattern=r"Nethya_", run_async=True)
